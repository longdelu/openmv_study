...
#ifdef FTL_DEBUG
#define FTL_DEBUG_INFO(...)    (int)printf(__VA_ARGS__)
#else
#define FTL_DEBUG_INFO(...)
#endif
...
u8 FTL_Init(void)
{
    ...
    if(temp) 
    {   
        FTL_DEBUG_INFO("format nand flash...\r\n");
        temp=FTL_Format();     //格式化NAND
        if(temp)
        {
            FTL_DEBUG_INFO("format failed!\r\n");
            return 2;
        }
    }else 	//创建LUT表成功
	{
		FTL_DEBUG_INFO("total block num:%d\r\n",nand_dev.block_totalnum);
		FTL_DEBUG_INFO("good block num:%d\r\n",nand_dev.good_blocknum);
		FTL_DEBUG_INFO("valid block num:%d\r\n",nand_dev.valid_blocknum);
    }
	return 0;
} 
...
u8 FTL_CopyAndWriteToBlock(u32 Source_PageNum,u16 ColNum,u8 *pBuffer,u32 NumByteToWrite)
{
    ...
    pageoffset=Source_PageNum%nand_dev.block_pagenum;	//获得页在所在块内的偏移，表示是当块第几个页 
...
retry:      
    ...
    {                                                                                                                                                                                                                                                                                                                                                                                                                                                       
        if(i>=pageoffset&&NumByteToWrite)				//有新的数据需要写入到当前页
        { 
 		...
 		}else								//无数据写入,直接拷贝即可，表明此时数据NumByteToWrite已经为0，或者新数据的地址在后面些，当前地址不用加入新数据
		{
		...
		{ 
			...
			goto retry;                     //重新把当前的数据拷到保留区里面一个有效的空闲区
		}
    } 
    ...
    if(i==nand_dev.block_pagenum) 			//拷贝完成
    {
        ...
        NAND_EraseBlock(source_block);		//擦除源块,重新作为保留区的一部分
...
u8 FTL_WriteSectors(u8 *pBuffer,u32 SectorNo,u16 SectorSize,u32 SectorCount)
{
	...
	for(i=0;i<SectorCount;i++)
    {
		...
		//读出写入大小的内容判断是否全为0XFF
		flag=NAND_ReadPageComp(PhyPageNo,PageOffset,0XFFFFFFFF,wlen/4,&temp);		//读一个wlen/4大小个数据(字读）,并与0XFFFFFFFF对比
		if(flag)return 2;						//读写错误
		...
		if(flag)//不全为0XFF/标记失败，将数据搬运到空闲块中，并在空闲块对应的页地址写上新的数据
        {
...
u8 FTL_ReadSectors(u8 *pBuffer,u32 SectorNo,u16 SectorSize,u32 SectorCount)
{
    ...
    for(i=0;i<SectorCount;i++)
    {
        ...
        BlockOffset=((SectorNo+i)%(nand_dev.block_pagenum*(nand_dev.page_mainsize/SectorSize)))*SectorSize;//计算块内偏移
        PhyPageNo=PBNNo*nand_dev.block_pagenum+BlockOffset/nand_dev.page_mainsize;	//计算出物理页号
        PageOffset=BlockOffset%nand_dev.page_mainsize;                     			//计算出页内偏移地址，把块内偏移转换成页内偏移
		rsecs=(nand_dev.page_mainsize-PageOffset)/SectorSize;						//计算在一个页内最多可以读取多少个扇区
...
u8 FTL_CreateLUT(u8 mode)
{
    ...
    {
        nand_dev.lut[i]=0XFFFF;                 // 数组下标是逻辑的单元地址，记录当前逻辑单元索引所在块的地址为特理地址，
    }	
    ...
    for(i=0;i<nand_dev.block_totalnum;i++)
    {
		...
		{
			FTL_DEBUG_INFO("bad block index:%d\r\n",i);
		}
    } 
    //LUT表建立完成以后检查有效块个数
    for(i=0;i<nand_dev.block_totalnum;i++)
...
u8 FTL_BlockCompare(u32 blockx,u32 cmpval)
{
	...
	for(i=0;i<3;i++)//允许3次机会
	{
		...
		if(res)FTL_DEBUG_INFO("error erase block:%d\r\n",i);
		else
		{ 
	...
	FTL_DEBUG_INFO("bad block checked:%d\r\n",blockx);
	return 1;
}
