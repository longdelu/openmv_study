...
//V1.5 20180523
//1.加入读状态命令发送时，延时tWHR的时间，以免出错
//2.不使用RB读IO引脚，读nandflash的忙状态判断该为读状态寄存器
//3.擦除时增加ms级别的延时，防止擦除时间不够立马进行读状态而造成出错
//4.增加tPROG的微妙延时，防止出错							  
////////////////////////////////////////////////////////////////////////////////// 	
...
#define NAND_TWHR_DELAY				  80			//tWHR等待延迟,最少60ns
#define NAND_TRHW_DELAY				  150			//tRHW等待延迟,最少100ns
#define NAND_TPROG_DELAY			  450			//tPROG等待延迟,最少220us,最大需要600us
#define NAND_TBERS_DELAY			  4			    //tBERS等待延迟,最少0.7ms,最大需要4ms
#define NAND_TR_DELAY			      25			//tR等待延迟,最大需要25us
#define NAND_TRST_DELAY			      500			//tRST复位后的等待时间，最大不超过500us,最小为5us
#define NAND_TRST_FIRST_DELAY		  3			    //tRST复位后的第一次等待时间，最大为3ms
#define NAND_INIT_DELAY			      200			//上电初始化就绪时间，最少200us


#define NAND_DEBUG                   

#ifdef NAND_DEBUG
#define NAND_DEBUG_INFO(...)    (int)printf(__VA_ARGS__)
#else
#define NAND_DEBUG_INFO(...)
#endif


//NAND延时,实际100个计数达到了2us左右
...
u8 NAND_Init(void)
{
    ...
    NAND_Handler.Init.ECCPageSize=FMC_NAND_ECC_PAGE_SIZE_512BYTE;       //ECC页大小为512
    ...
    ComSpaceTiming.HiZSetupTime=2;      //高阻态时间
    
    AttSpaceTiming.SetupTime=2;         //建立时间
    AttSpaceTiming.WaitSetupTime=3;     //等待时间
    AttSpaceTiming.HoldSetupTime=2;     //保持时间
    AttSpaceTiming.HiZSetupTime=2;      //高阻态时间
		 
    HAL_NAND_Init(&NAND_Handler,&ComSpaceTiming,&AttSpaceTiming);
    
    delay_us(NAND_INIT_DELAY);          //没有监控R/B引脚信号，上电延时200us确保nand器件在就绪状态 
	
    NAND_Reset();       		        //复位NAND
	
    delay_ms(NAND_TRST_FIRST_DELAY);    //复位等待不少于1个ms进入就绪状态
	
    nand_dev.id=NAND_ReadID();	        //读取ID
	
	NAND_DEBUG_INFO("nand flash id is 0x%x\r\n", nand_dev.id);
    ...
    else if (nand_dev.id==H27U4G8F2ETR) {
...
u8 NAND_ReadPage(u32 PageNum,u16 ColNum,u8 *pBuffer,u16 NumByteToRead)
{
    ...
    *(vu8*)(NAND_ADDRESS|NAND_CMD)=NAND_AREA_TRUE1;
    ...
    //代码换成延时函数,只不过这里我们为了效率所以没有用延时函数,时间大概不超过30us。
...
//    if(res)return NSTA_TIMEOUT;	//超时退出
    ...
    delay_us(NAND_TR_DELAY);        //等待25us

    // 发完0x00-0x30命令之后，可以中间插入读状态命令
    if(NAND_WaitForReady()==NSTA_READY) 
    {
        *(vu8*)(NAND_ADDRESS|NAND_CMD)=NAND_AREA_A;//成功，nand器件状态就绪,重新发送读模式，开始接收读回来的数据 
    }
    ...
    else
	{
		...
		eccstart=ColNum/NAND_ECC_SECTOR_SIZE;               //得到ecc计算缓冲区的起始地址 
		p=pBuffer;
        ...
        NAND_Delay(NAND_TRHW_DELAY);                        //随机读指令发送前需等待tRHW 
		...
		*(vu8*)(NAND_ADDRESS|NAND_CMD)=0XE0;				//开始读数据
		...
		for(i=0;i<eccnum;i++)								//检验ECC
		{
			...
			{
				NAND_DEBUG_INFO("err hd,rd:0x%x,0x%x\r\n",nand_dev.ecc_hdbuf[i+eccstart],nand_dev.ecc_rdbuf[i+eccstart]); 
 				NAND_DEBUG_INFO("eccnum,eccstart:%d,%d\r\n",eccnum,eccstart);	
				NAND_DEBUG_INFO("PageNum,ColNum:%d,%d\r\n",PageNum,ColNum);	
...
u8 NAND_WritePage(u32 PageNum,u16 ColNum,u8 *pBuffer,u16 NumByteToWrite)
{
    ...
    *(vu8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(PageNum>>16);
    ...
    else
    {
        ...
        eccstart=ColNum/NAND_ECC_SECTOR_SIZE;               //得到ecc计算保存的起始地址 
        for(res=0;res<eccnum;res++)
        {
            FMC_Bank2_3->PCR3|=1<<6;						//使能ECC校验 
...
//        *(vu8*)(NAND_ADDRESS|NAND_CMD)=NAND_WRITE_TURE1; 
        ...
        i=nand_dev.page_mainsize+0X10+eccstart*4;			//计算写入ECC的spare区地址，由于是512个字节就用了一个ecc，所以每次写入新的512字节时，spare区缓存ecc的地址都要加4. 
        *(vu8*)(NAND_ADDRESS|NAND_CMD)=0X85;				//随机写指令,改变写入数据的地址，允许在中间插入
        //发送地址
        *(vu8*)(NAND_ADDRESS|NAND_ADDR)=(u8)i;
        *(vu8*)(NAND_ADDRESS|NAND_ADDR)=(u8)(i>>8);
        NAND_Delay(NAND_TADL_DELAY);//等待tADL 
...
void NAND_EraseChip(void)
{
    ...
    for(i=0;i<nand_dev.block_totalnum;i++) //循环擦除所有的块
    {
        ...
        if(status)NAND_DEBUG_INFO("Erase %d block fail!!，错误码为%d\r\n",i,status);//擦除失败
    }
}

//获取ECC的奇数位的校验值/偶数位的校验值
//oe:0,偶数位
//   1,奇数位
//eccval:输入的ecc值
//返回值:计算后的ecc值(最多16位)
...
u8 NAND_ECC_Correction(u8* data_buf,u32 eccrd,u32 ecccl)
{
    ...
    if(eccchk==0XFFF)	//全1,说明只有1bit ECC错误
    {
        ...
        NAND_DEBUG_INFO("errorpos:%d\r\n",errorpos); 
        bytepos=errorpos/8; 
        data_buf[bytepos]^=1<<(errorpos%8);
    }
	else				//不是全1,说明至少有2bit ECC错误,无法修复
    {
        NAND_DEBUG_INFO("2bit ecc error or more\r\n");
        return 1;
    } 
    return 0;
}
